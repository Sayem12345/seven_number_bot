...
# --- Performance: reuse HTTP connections ---
...
def mask_number(phone):
    ...
    if len(phone_str) > 7: return f"{phone_str[:3]}VIP{phone_str[-4:]}"
    return phone_str
    ...
    with data_lock:
        if not os.path.exists(DATA_FILE):
            default_data = {
                ...
                "main_otp_link": "https://t.me/", "watermark": "\ud835\ude34\uaaC0\uaadc\uaaC0\uaac0",
        ...
        try:
            with open(DATA_FILE, "r", encoding='utf-8') as f:
                ...
                if not content:
                    return {"users": [], "services_data": {}, "forward_groups": [], "main_otp_link": "https://t.me/", "watermark": "\ud835\ude34\uaaC0\uaadc\uaaC0\uaac0", "force_join_enabled": False, "force_join_channels": []}
...
def show_force_join_message(chat_id, message_id=None):
    data = load_data()
    channels = data.get("force_join_channels", [])
    text = (f"{emo('warning')} <b>ACCESS DENIED</b> {emo('warning')}\n"
            f"━━━━━━━━━━━━━━━━━━\n"
...
def get_main_menu(user_id):
    ...
    markup.add(KeyboardButton("📱 GET NUMBER"), KeyboardButton("📊 TRAFFIC"))
    if user_id == ADMIN_ID: markup.add(KeyboardButton("⚙️ ADMIN PANEL"))
    return markup
...
def handle_text(message):
    ...
    if "GET NUMBER" in text:
        ...
        else: bot.send_message(message.chat.id, f"{emo('warning')} <b>Access Denied!</b>", parse_mode="HTML")
...
def show_main_menu(chat_id, first_name=None, message_id=None):
    if not first_name:
        ...
        except: first_name = "User"
    data = load_data()
    watermark = data.get("watermark", BOT_NAME)
    text = (f"{emo('crown')} <b>{html.escape(BOT_NAME)}</b> {emo('crown')}\n"
            f"━━━━━━━━━━━━━━━━━━\n"
...
def show_traffic_search(chat_id, message_id=None):
    ...
    text = (f"{emo('traffic')} <b>TRAFFIC CHECK</b> {emo('traffic')}\n━━━━━━━━━━━━━━━━━━\n"
            f"Type service name (e.g. WhatsApp)\n\n📝 Note: Facebook = Instagram Range\n\nSend /cancel to stop")
    if not message_id:
...
def show_user_ranges(chat_id, srv_id, cnt_id, message_id=None):
    ...
    text = (f"{emo('number')} <b>SELECT RANGE</b> {emo('number')}\n━━━━━━━━━━━━━━━━━━\n"
...
def show_admin_panel(chat_id, message_id=None):
    data = load_data()
    text = (f"{emo('crown')} <b>ADMIN PANEL</b> {emo('crown')}\n━━━━━━━━━━━━━━━━━━\n"
...
def handle_query(call):
    ...
    if call.data == "check_join":
        if check_force_join(user_id):
            bot.answer_callback_query(call.id, f"✅ Welcome to {BOT_NAME}!", show_alert=True)
    ...
    if call.data == "back_to_admin": show_admin_panel(chat_id, msg_id)
        ...
        srv_id = call.data.split("|")[1]
        if srv_id in data.get("services_data", {}): del data["services_data"][srv_id]
        save_data(data)
...
def process_api_traffic_search(message, msg_id):
    ...
    def check_traffic():
        ...
        try:
            ...
            response = _session.get(f"{BASE_URL}/api/v1/console/logs?limit=200", headers=headers, timeout=10)
...
def step_addgrpbtn_url(message, msg_id):
    ...
    grp_id, btn_name = state.get('grp_id'), state.get('btn_name')
...
def run_broadcast(chat_id, original_message, msg_id):
    ...
    safe_send(chat_id, f"{emo('mega')} <b>Broadcast Done!</b>\n\n{emo('done')} Sent: {success}\n{emo('cross')} Failed: {failed}", markup, msg_id)
...
def fetch_number(chat_id, service_info, api_key, msg_id, is_custom=False):
    ...
    try:
        response = _session.post(f"{BASE_URL}/api/v1/numbers/get", json=payload, headers=headers, timeout=15)
        res_data = response.json()
        if res_data.get("success"):
            ...
            text = (f"{emo('crown')} <b>NUMBER ALLOCATED</b> {emo('crown')}\n━━━━━━━━━━━━━━━━━━\n"
...
def poll_otp(chat_id, number_id, phone_number, service_info, api_key, msg_id, is_custom):
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            res = _session.get(f"{BASE_URL}/api/v1/numbers/{number_id}/sms", headers=headers, timeout=15)
            s_data = res.json()
            if s_data.get("success") and s_data.get("otp"):
                ...
                success_text = (f"{emo('done')} <b>COMPLETED</b> {emo('done')}\n━━━━━━━━━━━━━━━━━━\n"
                    f"{srv_emoji} Service: <b>{html.escape(service_info['service_name'])}</b>\n"
                    f"{flag} Country: <b>{html.escape(service_info['country_name'])}</b>\n━━━━━━━━━━━━━━━━━━\n"
...
if __name__ == "__main__":
    print(f"\ud835\ude34\uaaC0\uaadc\uaaC0\uaac0 - Premium Bot Running...")
